...
def change_w_out(arr_W_Out, gamma_Out, arr_Hidden_Y, amount_Hidden, alpha):
    for i in range(amount_Hidden):
...
def change_w_Hidden(arr_Hidden_W, gamma_Hidden, arr_Hidden_Y, arr_RefValues, amount_Input, amount_Hidden, iter, alpha):
    for i in range(amount_Hidden):
        for j in range(amount_Input):
            arr_Hidden_W[i][j] -= (
                alpha
...
def change_T_Hidden(arr_Hidden_T, gamma_Hidden, arr_Hidden_Y, amount_Hidden, alpha):
    for i in range(amount_Hidden):
...
def main():
    ...
    while error > min_error:
        ...
        for iter in range(amount_Train - amount_Input):
            ...
            for i in range(len(arr_Hidden_Y)):
                y_hid_arr_sum_square += arr_Hidden_Y[i] ** 2
            alpha_Out = 1 / y_hid_arr_sum_square

            alpha_Hidden = new_alpha_hid(arr_Hidden_Y, y, gamma_Hidden)

            arr_W_Out = change_w_out(arr_W_Out, gamma_Out, arr_Hidden_Y, amount_Hidden, alpha_Out)
            t_Out = change_t_Out(t_Out, gamma_Out, alpha_Out)
            arr_Hidden_W = change_w_Hidden(
                arr_Hidden_W, gamma_Hidden, arr_Hidden_Y, arr_RefValues, amount_Input, amount_Hidden, iter, alpha_Hidden
            )
            arr_Hidden_T = change_T_Hidden(arr_Hidden_T, gamma_Hidden, arr_Hidden_Y, amount_Hidden, alpha_Hidden)
