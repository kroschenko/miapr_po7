...
weight_i_k = [random.uniform(-1, 1) for _ in range(hidden_neuron)]
...
def Wjk_change(y_hidden, error,step_k):
    for i in range(hidden_neuron):
        weight_i_k[i] -= step_k * error * y_hidden[i]
...
def adaptive_step(y_hidden, y_out, error):
    ...
    error_j = []
    for i in range(hidden_neuron):
        error_j.append(error * y_out * (1 - y_out) * weight_i_k[i])
    for i in range(hidden_neuron):
