...
def change_w_hid(w_hid_arr, gamma_hidden, y_hid_arr, ref_arr, input_nn, hidden_nn, iter):
    alpha = 0.1
    for i in range(hidden_nn):
        for j in range(input_nn):
            w_hid_arr[i][j] -= alpha * gamma_hidden[i] * y_hid_arr[i] * (1 - y_hid_arr[i]) * ref_arr[j + iter]
    return w_hid_arr
...
def main():
    ...
    while err > min_err:
        ...
        for iter in range(train_value - input_nn):
            ...
            w_hid_arr = change_w_hid(w_hid_arr, gamma_hidden, y_hid_arr, ref_arr, input_nn, hidden_nn, iter)
