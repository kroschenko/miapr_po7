...
def change_w_hid(w_hid_arr, gamma_hidden, y_hid_arr, ref_arr, input_nn, hidden_nn, iter, alpha_hid):
    for i in range(hidden_nn):
        for j in range(input_nn):
            w_hid_arr[i][j] -= alpha_hid * gamma_hidden[i] * y_hid_arr[i] * (1 - y_hid_arr[i]) * ref_arr[j + iter]
    return w_hid_arr
...
def new_alpha_hid(y_hid_arr, y, gamma_hidden):
    ...
    for i in range(len(y_hid_arr)):
        ...
        denum += (gamma_hidden[i] ** 2) * (y_hid_arr[i] ** 2) * ((1 - y_hid_arr[i]) ** 2)
...
def main():
    ...
    while err > min_err:
        ...
        for iter in range(train_value - input_nn):
            y_hid_arr = calc_S_hid(y_hid_arr, w_hid_arr, ref_arr, T_hid_arr, input_nn, hidden_nn, iter)
            for i in range(hidden_nn):
            ...
            w_out_arr = change_w_out(w_out_arr, gamma_out, y_hid_arr, hidden_nn, alpha_out)
            T_out = change_T_out(T_out, gamma_out, alpha_out)
            w_hid_arr = change_w_hid(w_hid_arr, gamma_hidden, y_hid_arr, ref_arr, input_nn, hidden_nn, iter, alpha_hid)
            T_hid_arr = change_T_hid(T_hid_arr, gamma_hidden, y_hid_arr, hidden_nn, alpha_hid)
    ...
    ref_test_arr = [ref(x * step) for x in range(train_value - input_nn, test_value + train_value)]
    print("Testing result")
    print("{:^25}{:^25}{:^25}".format("Ref value", "Getting value", "Deviation"))
    for iter in range(test_value):

        y_hid_arr = calc_S_hid(y_hid_arr, w_hid_arr, ref_test_arr, T_hid_arr, input_nn, hidden_nn, iter)
        for i in range(hidden_nn):
            y_hid_arr[i] = calc_y_hid(y_hid_arr[i])
