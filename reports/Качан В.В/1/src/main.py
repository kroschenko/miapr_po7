...
def my_output(weights: list[float], t: float, inputs_number: int, training_output: list[float], shift: int) -> float:
    output = 0
    ...
    while error > min_error:
        ...
        for i in range(training_number - inputs_number):
            ...
            for j in range(inputs_number):
                ...
                weights[j] -= training_speed * (output_ - ideal_output) * training_output[i + j]
...
def main():
    ...
    testing_number = 19
    step = 0.1
    training_output = [func(i * step, a, b, d) for i in range(training_number + inputs_number)]
    testing_output = [func(i * step, a, b, d) for i in range(training_number, training_number + testing_number)]

    training_weight, training_t, data_for_drawing = training(inputs_number, training_number, training_output)
    ...
    for i in range(training_number - inputs_number):
        output_ = my_output(training_weight, training_t, inputs_number, training_output, i)
        print(f'{training_output[i + inputs_number]:<25} {output_:<25} {(training_output[i + inputs_number] - output_)}')

    print('\n\nРезультаты тестирования:')
    for i in range(testing_number - inputs_number):
        output_ = my_output(training_weight, training_t, inputs_number, testing_output, i)
        print(f'{testing_output[i + inputs_number]:<25} {output_:<25} {(testing_output[i + inputs_number] - output_)}')
