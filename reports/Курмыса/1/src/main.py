...
last_err = 0 # для того чтобы прервать обучение при том случае, когда погрешности не изменятся
alpha = float(input('Введите скорость обучения: '))

print('Изначальные данные:\nИзначальные коэфф. -', omega, '\nПорог функции T -', T, '\nШаг функции alpha -', alpha)
...
while test_number <= 15:
  ...
  t = f(X) # идеальное значение функции
  ...
  err = delta**2 / 2
  error_sum += err
  print(f'Эпоха # {test_number:2}: {t:21} {y:21} {delta:24} {err:24}')
  test_number += 1
