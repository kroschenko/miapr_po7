...
int main() {
	...
	while (1) {
		...
		double speed = 0.01; //скорость обучения
		...
		for (int i = 0; i < n - enteries; i++) {
			...
			for (int j = 0; j < enteries; j++) { //изменение весовых коэффициентов
				W[j] -= speed * (y1 - etalon_values[i + enteries]) * etalon_values[i + j];
			}

			T += speed * (y1 - etalon_values[i + enteries]); //изменение порога нейронной сети
